...
static int dl_dci_to_grant_mcs(srslte_ra_dl_dci_t *dci, srslte_ra_dl_grant_t *grant, bool crc_is_crnti) {
  ...
  if (!crc_is_crnti) {
    ...
    if ((dci->dci_format == SRSLTE_RA_DCI_FORMAT2) || (dci->dci_format == SRSLTE_RA_DCI_FORMAT2A)) {
    	if  ((dci->mcs_idx2==0) && (dci->rv_idx2==1)) {
    		if (tbs < 0) {
			  return SRSLTE_ERROR;
			} else {
			  grant->mcs.tbs = (uint32_t) tbs;
			  grant->mcs2.tbs = (uint32_t) 0;
			  //printf("double sized (1d) %d, %d\n",tbs,tbs2);
			  return SRSLTE_SUCCESS;
			}
    	} else {
    		if (dci->mcs_idx2 < 10) {
			  grant->mcs2.mod = SRSLTE_MOD_QPSK;
			  i_tbs2 = dci->mcs_idx2;
			  tbs2 = srslte_ra_tbs_from_idx(i_tbs2, n_prb);
			} else if (dci->mcs_idx2 < 17) {
			  grant->mcs2.mod = SRSLTE_MOD_16QAM;
			  i_tbs2 = dci->mcs_idx2-1;
			  tbs2 = srslte_ra_tbs_from_idx(i_tbs2, n_prb);
			} else if (dci->mcs_idx2 < 29) {
			  grant->mcs2.mod = SRSLTE_MOD_64QAM;
			  i_tbs2 = dci->mcs_idx2-2;
			  tbs2 = srslte_ra_tbs_from_idx(i_tbs2, n_prb);
			} else if (dci->mcs_idx2 == 29) {
			  grant->mcs2.mod = SRSLTE_MOD_QPSK;
			  tbs2 = 0;
			  i_tbs2 = 0;
			} else if (dci->mcs_idx2 == 30) {
			  grant->mcs2.mod = SRSLTE_MOD_16QAM;
			  tbs2 = 0;
			  i_tbs2 = 0;
			} else if (dci->mcs_idx2 == 31) {
			  grant->mcs2.mod = SRSLTE_MOD_64QAM;
			  tbs2 = 0;
			  i_tbs2 = 0;
			}
			if (tbs2 < 0 || tbs < 0) {
			  return SRSLTE_ERROR;
			} else if ((dci->mcs_idx==0) && (dci->rv_idx==1)) {
	    		if (tbs2 < 0) {
				  return SRSLTE_ERROR;
				} else {
				  grant->mcs2.tbs = (uint32_t) tbs2;
				  grant->mcs.tbs = (uint32_t) 0;
				  //printf("double sized (2d) %d, %d\n",tbs,tbs2);
				  return SRSLTE_SUCCESS;
				}
			} else {
			  if (tbs != tbs2) {
